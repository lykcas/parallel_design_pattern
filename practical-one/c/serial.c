...
int main(int argc, char* argv[]) {
  ...
  MPI_Status stats;
  MPI_Request reqs;
  MPI_Init(&argc, &argv);  // We just have this so we can use MPI Wtime in the
                           // serial code for timing
  MPI_Comm_size(comm, &size);
  MPI_Comm_rank(comm, &rank);
  // int dims[1] = {0};
  // MPI_Dims_create(size, 1, dims);
  int periods[1] = {1}, coords[1];
  int up_nbr, down_nbr;
  MPI_Cart_create(comm, 1, &size, periods, 0, &comm);
  ...
  if (argc != 5) {
    printf(
        ...
        "the global size in Y, convergence accuracy and max number "
        "iterations\n");
    printf(
        "In the absence of this defaulting to x=128, y=1024, convergence=3e-3, "
...
#ifdef INSTRUMENTED
  // Later in the exercise we will be tracing the execution, for large numbers
  // of iterations this can result in large file sizes - hence we have a safety
  // check here and limit the number of iterations in this case (which still
  // illustrates the parallel behaviour we are interested in.)
  ...
  int flag = 0;
  if (rank < nx - local_nx * size) {
    local_nx++;
    flag = 1;
  }
  int last_rows;
  if (flag == 1) {
    last_rows = local_nx * rank;
  } else {
    last_rows = (local_nx + 1) * (nx % size) + (rank - nx % size) * local_nx;
  }

  double* small_u_k = malloc(sizeof(double) * mem_size_x * local_nx);
  ...
  for (j = 1; j <= nx; j++) {
    for (i = 1; i <= ny; i++) {
      bnorm = bnorm + pow(u_k[i + (j * mem_size_y)] * 4 -
  ...
  // In the parallel version you will be operating on only part of the domain in
  // each process, so you will need to do some form of reduction to determine
  // the global bnorm before square rooting it
  bnorm = sqrt(bnorm);
  ...
  for (k = 0; k < MAX_ITERATIONS; k++) {
    ...
    for (j = 1; j <= nx; j++) {
      for (i = 1; i <= ny; i++) {
        rnorm = rnorm + pow(u_k[i + (j * mem_size_y)] * 4 -
                                u_k[(i - 1) + (j * mem_size_y)] -
                                u_k[(i + 1) + (j * mem_size_y)] -
                                u_k[i + ((j - 1) * mem_size_y)] -
                                u_k[i + ((j + 1) * mem_size_y)],
    ...
    // In the parallel version you will be operating on only part of the domain
    // in each process, so you will need to do some form of reduction to
    // determine the global rnorm before square rooting it
    ...
    if (k % REPORT_NORM_PERIOD == 0)
      // Reduce norm here but can combine this part with line 106
